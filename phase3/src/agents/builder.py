    ...
    if retry > 0 and "debug_analysis" in state:
        ...
        if state.get("is_paradigm_shift"):
            context += (
                ...
                f"You are stuck in a failure loop. COMPLETELY DISCARD your previous patches.\n"
                f"Debug Analysis:\n{dbg.root_cause_analysis}\n"
                f"Proposed Fix Strategy:\n{dbg.proposed_fix_strategy}\n"
                f"\nStart writing a new patch from scratch based on this radically different strategy."
            )
        else:
            context += (
